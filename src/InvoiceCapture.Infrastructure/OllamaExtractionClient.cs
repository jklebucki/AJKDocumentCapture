...
public sealed class OllamaExtractionClient(HttpClient httpClient, IOptions<OllamaOptions> options) : IInvoiceExtractionClient
{
    ...
    private static readonly object InvoiceSchema = new { type = "object", additionalProperties = false, properties = new { documentType = new { type = "string" }, invoiceNumber = new { type = new[] { "string", "null" } }, sellerNip = new { type = new[] { "string", "null" } }, issueDate = new { type = new[] { "string", "null" } }, currency = new { type = "string" }, sourceBlockIds = new { type = "array", items = new { type = "string" } } }, required = new[] { "documentType", "invoiceNumber", "sellerNip", "issueDate", "currency", "sourceBlockIds" } };
}
